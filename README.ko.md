...
**버틴 것과 못 버틴 것:** 프로바이더 추상화([프로바이더 추가](#프로바이더-추가))는 호출 계층에서는 제 역할을 했습니다 — 새 프로바이더는 독립된 파일 하나와 레지스트리 한 줄이면 됩니다. 다만 설정 계층에서 멈춥니다. 설정 마법사가 프로바이더 이름과 키 필드를 하드코딩하고 있어서, 세 번째 프로바이더를 붙이려면 그쪽도 고쳐야 합니다. 이 두 관심사를 분리하는 게 다시 한다면 바꿀 부분입니다. 파싱 fallback 체인과 설정 우선순위 규칙은 다른 프로젝트에 그대로 가져갈 만합니다.
